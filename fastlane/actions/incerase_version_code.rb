module Fastlane
  module Actions
    class IncreaseVersionCodeAction < Action
      def self.run(params)
        version_code = "0"
        ...
        if gradle_file_path != nil
            UI.message("incerase_version_code plugin will use gradle file at (#{gradle_file_path})!")
            version_code = getVersionCode(gradle_file_path, constant_name)
            UI.message("Increase version here."
            version_file = File.open("#{gradle_file_path}")
            version_file.each do |line|
              version_file.replace_puts('            versionCode      : #{versionCode} + 1,') if line =~ /versionCode/
            end
            version_file.close
        else
            app_folder_name ||= params[:app_folder_name]
            UI.message("The incerase_version_code plugin is looking inside your project folder (#{app_folder_name})!")
      ...
      def self.description
        "Increase the version code of an Android project. This action will return the version code of your project according to the one set in your build.gradle file"
      end
